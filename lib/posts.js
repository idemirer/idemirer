...
export function getSortedPostsData() {
  ...
  const finalPosts = allPostsData.filter(({ draft, date }) => {
    const timeZoneOffset = new Date(Date.parse(date)).getTimezoneOffset() * 60 * 1000;
    return (draft !== true) & (today >= Date.parse(date) + timeZoneOffset);
  }); // Dates are 2022-12-01 format. Sometimes they are parsed at 7pm.
